...
export class CratesIoDownloadsCounter implements DownloadsCounter {
  ...
  async getCount(): Promise<DownloadsCounterResult> {
    const response = await fetch(
      `https://crates.io/api/v1/crates/${this.packageName}`,
      {
        headers: {
          "User-Agent": "marcinjahn.com build",
        },
      },
    );
    ...
    if (!response.ok) {
      throw new Error(
        `crates.io downloads counter failed for ${this.packageName}. Error code: ${response.status}`,
      );
    }
