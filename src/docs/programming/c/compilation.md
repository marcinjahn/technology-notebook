...
Our source files go through the following process:
...
all the lines that start with the `#` (e.g., `#include` or `#if`).
...
(`.so` files). An example of it could be "iostream".
...
When compiling programs, we can specify expkicitly the dynamic libraries that we
