...
1. "example.com" is a domain name. The networking stack needs IP address, so
   we're going to reach out to DNS for the IP address
    ...
    4. DNS respons to us with the IP address. The IP address could be found either
      ...
      for the IP.
2. With the IP address, we can start to establish a [TCP connection](/technologies/networking/tcp/) with
   the "example.com" server.
    1. 3-way handshake occurs and sockets are created on the client and the server
      (IP-port pairs).
3. [HSTS](/technologies/networking/hsts/) can jump in forcing us to use HTTPS (let's assume that it
   ...
   version used, a proper handshake procedure will occur.
    - if TLS 1.3 is used:
        ...
        2. The server responds with "Server Hello" including: D-H parameters,
          [X.509
          certificate](/technologies/security/cryptography/basic-terms#x-509)
          (based on SNI).
    - if TLS 1.2 is used:
...
- Use [HSTS](/technologies/networking/hsts/) - it's a header that informs the browser to ALWAYS use HTTPS when
  reaching the app.
