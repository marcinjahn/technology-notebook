...
SELinux is an implementation of a MAC (Mandatory Access Control) mechanism. 
It extends the default DAC (Discretionary Access Control).
...
We can display the list of allowed types for a given role with `seinfo -ruser_r -x` 
(`user_r` is the name of the role in this example).

A user can switch its role with `newrole -r somerole_r`. The supported roles are defined 
for each SELinux user.
