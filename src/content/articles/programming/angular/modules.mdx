...
tags: ["angular", "spa", "js", "ts", "modules", "organization", "dynamic", "ngmodule", "scope", "routes", "lazy loading", "preload", "dependency injection"]
lang: en-US
---
...
[Standalone Components](./standalone-components.mdx) do not require modules
to be there!
:::
...
- **imports** - here, we pull in other modules (or other things if [Standalone
  Components](./standalone-components) are being used), either provided by
...
- **entryComponents** - *deprecated*, it was used for [dynamic
  modules](./components#instantiating-components-from-typescript) in the
  past.
...
with some declared component, I can use `routerLink` in that component only if I icmport 
