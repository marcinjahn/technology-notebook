...
tags: ["angular", "spa", "js", "ts", "tips", "global styles", "ViewChild", "Renderer", "encapsulation", "environments"]
lang: en-US
---
...
hook](/programming/angular/components#component-lifecycle), the `@ViewChild` decorator requires
...
- replace that `EventEmitter<T>` with `Subject<T>` - it works pretty much the
  same, but is more functional (read
  [here](/programming/angular/observables#subject)). `EventEmitter<T>` would
  still work if for some reason we want to keep using that.
