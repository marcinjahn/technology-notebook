...
Application](/programming/functional-programming/partial-application), it makes
sense to order the arguments from the most generic ones to the most specific
ones.
...
similar to [Rust's `()` type](programming/rust/basics).
...
[honest](/programming/functional-programming/signatures#honesty).
...
[More about `Map` and
`ForEach`](/programming/dotnet/functional/fundamentals#map-vs-foreach)
:::