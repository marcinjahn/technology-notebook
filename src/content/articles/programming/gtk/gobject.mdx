...
instance of `GtkButtonClass`. There're even macros that allow us to retrieve a
...
not register the same type mulitple times, taking care of thread-safety, and so
on.
:::

Thanks to the registration, the instantation of the new objects of the
registered type will be simplified later on, and it will properly setup the new
