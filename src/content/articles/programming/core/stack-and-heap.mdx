...
- memory allocated on the heap is linked to via a pointer/reference placed on
  the stack. This pointer/reference can then be cloned or moved around between
  different functions.
...
larger amounts of data on the heap. The stack's size is pretty small compared to
the heap.

## Stack Machine

"Stack" is not just a region of memory. This word sometimes also refers to the
architecture of a Stack Machine. CPUs (and VMs) have built-in support for stacks,
...
function returns, its stack frame is gone (or rather, the stack pointer gets
decremented). Too many recursive calls will cause a *stack overflow* since the
stack has some predefined size.
...
- it's going to be used by multiple functions (placing it on the stack
...
how to allocate data on the heap, every program can have its way of doing that.
Allocators take care of things such as memory defragmentation.
